...
var get_dom = () => {
    ...
    function truncate(nodes) {
        ...
        for (i = 0; i < nodes.length; i++) {
            s = nodes[i].textContent;
            if (s && s.length > MAXLEN)
                nodes[i].textContent = s.substr(0, MAXLEN) + " truncated " + s.length + "bytes...";
        }
    }
...
var get_js = (a, a_html, mark_initial, show_onclick) => {
    var i, node;
...
var parse_cssrules = (cssNode, a, mark_initial, depth) => {
    if (depth > 10 || !cssNode)
        return;
...
var pick_node = (node, array, mark_initial) => {
    // skip extension scripts
    var src = node.href || node.src;
    if (src && startsWith("" + src, "moz-extension:"))
        return null;
