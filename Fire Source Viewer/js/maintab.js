...
var show_src = (li, show_whole_file) => {
...
var build_item = (item, lang, show_whole_file) => {
    if (is_whole_file)
        return;
...
var update_li_text = (item, header) => {
    var s = "";
    if (item.src) {
        if (item.data != undefined)
            s = bytesToSize(item.data.length);
    } else {
        s = bytesToSize(item.inline.length);
    }
...
var insert_line_numbers = () => {
    $("#src ol").empty();
    var nums = [],
        count = calculate_line_count();
